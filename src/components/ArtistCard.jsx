...
export default function ArtistCard({ name, label, bio, photo, songs = [] }) {
  ...
  return (
    <article
      ...
      style={{ transition: 'background-color 250ms cubic-bezier(0.22,1,0.36,1), transform 250ms cubic-bezier(0.22,1,0.36,1), border-color 250ms cubic-bezier(0.22,1,0.36,1)' }}
      onMouseEnter={e => {
        e.currentTarget.style.backgroundColor = '#521f2b'
        e.currentTarget.style.transform = 'translateY(-3px)'
        e.currentTarget.style.borderColor = 'rgba(227,209,184,0.95)'
      }}
      onMouseLeave={e => {
        e.currentTarget.style.backgroundColor = '#4a2030'
        e.currentTarget.style.transform = 'translateY(0)'
        e.currentTarget.style.borderColor = '#e3d1b8'
      }}
    >
      {/* ── Artist photo ── */}
      <div className="w-full overflow-hidden bg-[#3d1a22] relative" style={{ aspectRatio: '4/3' }}>
        {photo && !imgError ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-2">
            ...
            <p className="font-inter text-[9px] tracking-widest text-[#e3d1b8]/20 uppercase">Artist Photo</p>
          </div>
        )}
      ...
      <div className="flex flex-col flex-1 p-4">
        ...
        {/* Artist name */}
        <h3
          className="font-oswald font-bold text-[#e3d1b8] uppercase leading-tight mb-2"
          ...
          <ul className="space-y-[7px]">
            {songs.slice(0, 3).map((song, i) => {
              ...
              return (
                <li key={i}>
                  <a
                    href={spotifySearchUrl(name, title)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full flex items-center gap-2.5 no-underline group/song"
                  ...
                  >
                    {/* Play button */}
                    <span
                      className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center bg-[#1DB954]/20 text-[#1DB954]"
                      style={{ transition: 'background-color 200ms cubic-bezier(0.22,1,0.36,1)' }}
                      onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'rgba(29,185,84,0.4)' }}
                      onMouseLeave={e => { e.currentTarget.style.backgroundColor = 'rgba(29,185,84,0.2)' }}
                    >
                      <IconPlay />
                    </span>
                    ...
                    {/* Title */}
                    <span
                      className="font-inter text-[11px] truncate text-[#e3d1b8]/55"
                      style={{ transition: 'color 200ms cubic-bezier(0.22,1,0.36,1)' }}
                      onMouseEnter={e => { e.currentTarget.style.color = 'rgba(227,209,184,0.8)' }}
                      onMouseLeave={e => { e.currentTarget.style.color = 'rgba(227,209,184,0.55)' }}
                    >
                      {title}
                      {feat && <span className="text-[#e3d1b8]/35 ml-1">ft. {feat}</span>}
                    </span>

                    {/* Spotify icon */}
                    <span
                      className="flex-shrink-0 ml-auto text-[#1DB954]/40"
                      style={{ transition: 'color 200ms cubic-bezier(0.22,1,0.36,1)' }}
                      onMouseEnter={e => { e.currentTarget.style.color = 'rgba(29,185,84,0.8)' }}
